		...
		if(map.list.length > 0){
			...
			map.list.forEach(function(item, index){
				...
				content += "<a href = '/file/download?fileName="+savePath+"'>" + item.filename + '</a>' 
						+ '<i onclick="attachFileDelete(this)" data-bno="'+item.bno+'" data-uuid="'+item.uuid+'" class="fa-solid fa-trash-can"></i>'+'<br>';
			})
...
<main class="container">
  ...
  <input type='text' name="searchWord" value="${param.searchWord }">
	...
	<div class="mb-3">
	  <label for="writer" class="form-label">첨부 파일</label>
	...
	<button type="submit" class="btn btn-primary btn-lg" onclick="requestAction('/board/editAction')">수정하기</button>
	</c:if>
